  ...
  try {
    ...
    if (data.success) {
      if (data.loginIds && data.loginIds.length > 0) {
        ...
        resultCount.value = data.loginIds.length
        successMessage.value = data.message || (data.loginIds.length === 1 ? '?„ì´?”ë? ì°¾ì•˜?µë‹ˆ??' : `ì´?${data.loginIds.length}ê°œì˜ ?„ì´?”ë? ì°¾ì•˜?µë‹ˆ??`)
      } else {
        successMessage.value = data.message || '?„ì´??ì°¾ê¸° ?”ì²­??ì²˜ë¦¬?˜ì—ˆ?µë‹ˆ??'
      }
    } else {
      errorMessage.value = data.message || '?„ì´??ì°¾ê¸°???¤íŒ¨?ˆìŠµ?ˆë‹¤.'
    }
  } catch (err) {
    if (err.response) {
      ...
      if (errorData && errorData.message) {
        ...
        errorMessage.value = '?…ë ¥???´ë©”?¼ë¡œ ê°€?…í•œ ?„ì´?”ë? ì°¾ì? ëª»í–ˆ?µë‹ˆ??'
      } else {
        errorMessage.value = `?„ì´??ì°¾ê¸° ?¤íŒ¨ (status: ${err.response.status})`
      }
    } else {
      errorMessage.value = '?œë²„ ?°ê²° ?¤íŒ¨'
    }
  }
}
</script>
    ...
    <div class="grid h-full w-full grid-cols-1 md:grid-cols-[9fr_11fr]">
        ...
        <div class="relative z-[2]">
          <h1 class="m-0 text-[40px] font-bold leading-[1.2] tracking-[-1px] text-white md:text-[52px] lg:text-[60px]">
            <span class="inline-flex cursor-pointer items-center gap-2 transition-opacity hover:opacity-80" @click="goToHome">
              ...
              AutoSource
            </span><br /><span class="text-[34px] md:text-[44px] lg:text-[50px]">?„ì´??ì°¾ê¸°</span>
          </h1>
          <p class="mt-4 text-[18px] font-medium text-white/80 md:text-[20px]">
            ê°€?…í•œ ê³„ì •??br />?„ì´?”ë? ?•ì¸?˜ì„¸??
          </p>
        </div>
      ...
      <section class="flex flex-col justify-center overflow-y-auto bg-white px-6 py-8 md:px-12 lg:px-20">
        <div class="w-full max-w-[600px] mx-auto">
          ...
          <p class="mb-6 text-[16px] text-[#6b7280]">ê°€?…í•  ???¬ìš©???´ë©”?¼ì„ ?…ë ¥?´ì£¼?¸ìš”.</p>

          <!-- ?´ë©”??-->
          <div class="mb-6 flex items-center gap-3">
            <label class="w-[120px] flex-shrink-0 text-[17px] font-bold text-[#111827]">?´ë©”??/label>
            <input
              v-model="email"
              type="email"
          ...
          <div v-if="hasResult && resultIds.length === 1" class="mt-6 rounded-lg border border-[#e5e7eb] bg-white p-5">
            <p class="mb-2 text-[14px] font-bold text-[#6b7280]">ì°¾ì? ?„ì´??/p>
            <p class="text-[24px] font-bold text-[#111827]">{{ resultIds[0] }}</p>
          </div>

          <!-- ?¤ì¤‘ ê²°ê³¼ -->
          <div v-if="hasResult && resultIds.length > 1" class="mt-6 rounded-lg border border-[#e5e7eb] bg-white p-5">
            <p class="mb-3 text-[14px] font-bold text-[#6b7280]">ì°¾ì? ?„ì´??ëª©ë¡</p>
            <div class="space-y-2">
              <div
              ...
              >
                ...
                <span class="text-[13px] font-medium text-[#9ca3af]">ê³„ì • {{ index + 1 }}</span>
              </div>
            </div>
          </div>
        </div>
